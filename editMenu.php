                ...
                ?>
                    <form action="koneksi.php" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control mb-3" name="namaMenu" placeholder="Nama Menu" required value="<?= $row['namaMenu'] ?>">
                        </div>

                        <div class="form-group mt-3">
                            <label for="desk" class="text-light">Deskripsi</label>
                            <textarea class="form-control" name="desk" id="desk" cols="5" rows="3" placeholder="Deskripsi" required><?= $row['desk'] ?></textarea>
                        </div>

                        <div class="form-group mt-3">
                            <label for="gambar" class="text-light">Gambar</label>
                            <input type="file" class="form-control" name="gambar" id="gambar" aria-describedby="inputGroupFileAddon04" aria-label="Upload" value="<?= $row['gambar'] ?>" required>
                        </div>

                        <div class="form-group mt-3">
                            <label for="harga" class="text-light">Harga Menu</label>
                            <input type="number" class="form-control mb-3" name="harga" placeholder="Harga Menu" required value="<?= $row['harga'] ?>">
                        </div>
